...
一般客户是最大 RFM 客群；同期群在首购后快速流失，第 1、3、6 月加权精确留存率分别为 {month_1_retention:.2%}、{month_3_retention:.2%} 和 {month_6_retention:.2%}。

### 6.4 留存目标与客户行动

![Power BI 留存目标与客户行动](../docs/assets/powerbi_04_retention_actions.png)

行动队列把 cohort、RFM 客群、目标客户数、历史商品 GMV 和建议旅程放在同一视图中；其中 {len(ranked_retention_groups)} 个客群满足规模与可观察期门槛，应通过留出组验证增量效果。
...
州际延迟交付率差异明显；延迟订单平均评分为 {delivery.loc['late', 'average_review_score']:.2f}，显著低于准时订单的 {delivery.loc['on_time', 'average_review_score']:.2f}，支持按影响规模配置配送改进资源。
...
线路级队列按延迟订单量、延迟率和商品 GMV 排序。{top_lane_summary} 建议行动用于区分卖家发货 SLA 与承运线路容量、路由复核方向。
