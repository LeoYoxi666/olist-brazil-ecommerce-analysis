...
- `data/` 包含 11 张经过字段校验的全中文轻量聚合 CSV，总量小于 1 MB。
...
11 张带 UTF-8 BOM 的中文 CSV，使用 Import 模式。各表统计粒度不同，默认不要自动
创建表关系；每个页面使用对应聚合表，避免跨粒度重复汇总。

导入后完成以下设置：

1. 按 `build_guide.md` 的映射将 11 个英文文件名对应的查询改为中文表名。
2. 将含 `GMV`、`价格`、`价值` 的字段设置为货币 `R$` 或小数两位。
3. 将含 `率`、`占比`、`比例` 的字段设置为百分比、小数两位。
4. 将 `订单月份`、`首购同期群月份` 转为日期并升序排序。
5. 将排名、月序、订单和客户数量设置为整数；业务 ID 和州代码设为文本且不汇总。
6. 删除 Power BI 自动检测的关系，除非后续建立经过验证的共享维度表。
7. 页面名、图表标题、图例、坐标轴、切片器、卡片、表头和工具提示全部使用中文。
